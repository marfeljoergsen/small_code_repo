...
ARCH=$( grep ^NAME= /etc/os-release | cut -d'"' -f 2 | cut -d' ' -f1 )
case $ARCH in
  Arch)
  ...
  Debian|Raspbian)
    ...
    if [ $( apt list 2> /dev/null | grep hostapd | grep -cw installed ) -ne 1 ]; then
      ...
      apt-get update
      # then, install hostapd and dnsmasq packages
      apt-get install -y hostapd dnsmasq wireless-tools iw rfkill
    fi
    ;;
  *)
    echo "Running on unsupported OS [ $ARCH ].  Exiting."
    exit 1
    ;;
esac
...
if [ ! -e /etc/dnsmasq.conf.save ]; then 
  cp /etc/dnsmasq.conf /etc/dnsmasq.conf.save
  cat <<DNSCONF > /etc/dnsmasq.conf
